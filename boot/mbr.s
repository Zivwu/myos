...
rd_disk_m_16:	   
    ...
    out dx,al          ; 设置 out 用于将al数据发送到指定的输出dx端口
    mov eax,esi	       ; 恢复eax
    ...
    mov cl,8           ; ax右移八位   
    shr eax,cl		;一个寄存器或内存中的值进行逻辑右移操作cl位
...
;mul dx 是汇编语言中的指令，用于将 ax 寄存器中的值和 dx 寄存器中的值相乘，
;结果保存在 dx:ax 寄存器对中，即高 16 位保存在 dx 中，低 16 位保存在 ax 中。
;这里 dx 用来存储除数，ax 用来存储乘数，执行完 mul dx 后，
;dx 存储乘法结果的高 16 位，ax 存储乘法结果的低 16 位。
    mul dx
    mov cx, ax	       ; 将要读的次数传给cx
    mov dx, 0x1f0      ; 要读的端口号

.go_on_read:
; 从输入端口 dx 中读取数据到 al 寄存器
